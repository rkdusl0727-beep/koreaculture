...
function HiddenHeritage({soundOn,onBack,onComplete,onNext}:{soundOn:boolean;onBack:()=>void;onComplete:()=>void;onNext:()=>void}){
  ...
  const find=(treasure:HeritageTreasure)=>{const isNew=!found.includes(treasure.id);if(isNew){setFound(value=>[...value,treasure.id]);playDingDongDaeng(soundOn)}setFocus(treasure.id);setMessage(`${treasure.name}: ${treasure.desc}`);stopKoreanSpeech();stopRecordedVoice();if(soundOn){const audio=useNormalAudioSpeed(new Audio(`/audio/heritage/${treasure.id}.wav`));recordedVoice.current=audio;void audio.play().catch(()=>speakKorean(`${treasure.name}. ${treasure.desc}`,true))}};
...
function MemoryGame({count,soundOn,onBack,onComplete,onNext}:{count:number;soundOn:boolean;onBack:()=>void;onComplete:()=>void;onNext:()=>void}){
  ...
  useEffect(()=>{if(complete)onComplete()},[complete,onComplete]);
  const choose=(card:MemoryCard)=>{if(locked||hinting||open.includes(card.key)||matched.includes(card.treasure.id))return;speakKorean(card.treasure.name,soundOn);const next=[...open,card.key];setOpen(next);if(next.length<2)return;setLocked(true);const first=deck.find(value=>value.key===next[0])!;if(first.treasure.id===card.treasure.id){window.setTimeout(()=>{setMatched(value=>[...value,card.treasure.id]);setOpen([]);setLocked(false);setMessage(`같은 문화재를 찾았어요! ${card.treasure.name}`);playDingDong(soundOn)},450)}else{setMessage('두 모습을 천천히 비교해 보세요.');window.setTimeout(()=>{setOpen([]);setLocked(false)},1500)}};
