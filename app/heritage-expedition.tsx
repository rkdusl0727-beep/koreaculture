...
function newHiddenPlacements(previous:Record<string,number>={}){const used:Array<{x:number;y:number}>=[];return Object.fromEntries(HERITAGE_TREASURES.map(treasure=>{const candidates=shuffled(treasure.positions.map((point,index)=>({point,index})));const choice=candidates.find(({point,index})=>index!==previous[treasure.id]&&used.every(other=>Math.hypot(point.x-other.x,point.y-other.y)>8))||candidates.find(({index})=>index!==previous[treasure.id])||candidates[0];used.push(choice.point);return[treasure.id,choice.index]}))}
