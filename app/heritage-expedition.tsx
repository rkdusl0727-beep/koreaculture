...
function HiddenHeritage({soundOn,onBack,onComplete,onNext}:{soundOn:boolean;onBack:()=>void;onComplete:()=>void;onNext:()=>void}){
  ...
  useEffect(()=>()=>{if(speechTimer.current!==null)window.clearTimeout(speechTimer.current)},[]);
  const find=(treasure:HeritageTreasure)=>{const isNew=!found.includes(treasure.id);if(isNew){setFound(value=>[...value,treasure.id]);playDingDongDaeng(soundOn)}setFocus(treasure.id);setMessage(`${treasure.name}: ${treasure.desc}`);if(speechTimer.current!==null)window.clearTimeout(speechTimer.current);const read=()=>{speechTimer.current=null;speakKorean(`${treasure.name}. ${treasure.desc}`,soundOn)};if(isNew&&soundOn)speechTimer.current=window.setTimeout(read,620);else read()};
  const miss=()=>{if(speechTimer.current!==null){window.clearTimeout(speechTimer.current);speechTimer.current=null}const clue=HERITAGE_CLUES[Math.floor(Math.random()*HERITAGE_CLUES.length)];setFocus(null);setMessage(clue);speakKorean(clue,soundOn)};
  const reset=()=>{if(speechTimer.current!==null){window.clearTimeout(speechTimer.current);speechTimer.current=null}stopKoreanSpeech();setFound([]);setFocus(null);setPlacements(current=>newHiddenPlacements(current));setMessage('문화재의 위치가 자연스러운 장소 안에서 바뀌었어요!')};
...
function MemoryGame({count,soundOn,onBack,onComplete,onNext}:{count:number;soundOn:boolean;onBack:()=>void;onComplete:()=>void;onNext:()=>void}){
  ...
  useEffect(()=>{if(complete)onComplete()},[complete,onComplete]);
  const choose=(card:MemoryCard)=>{if(locked||hinting||open.includes(card.key)||matched.includes(card.treasure.id))return;const next=[...open,card.key];setOpen(next);if(next.length<2)return;setLocked(true);const first=deck.find(value=>value.key===next[0])!;if(first.treasure.id===card.treasure.id){window.setTimeout(()=>{setMatched(value=>[...value,card.treasure.id]);setOpen([]);setLocked(false);setMessage(`같은 문화재를 찾았어요! ${card.treasure.name}`);playDingDong(soundOn)},450)}else{setMessage('두 모습을 천천히 비교해 보세요.');window.setTimeout(()=>{setOpen([]);setLocked(false)},1500)}};
  ...
  return <section className="expedition-activity memory-activity"><div className="expedition-heading"><Button variant="outline" onClick={onBack}>← 이전</Button><div><h2>문화재 메모리게임</h2><p>{count===6?'쉬움 · 6종 12장':'도전 · 8종 16장'}</p></div><strong>{matched.length} / {count} 짝</strong></div><div className={`memory-grid cards-${count*2}`}>{deck.map(card=>{const visible=hinting||open.includes(card.key)||matched.includes(card.treasure.id);return <button key={card.key} className={`memory-card memory-${card.treasure.id} ${visible?'open':''} ${matched.includes(card.treasure.id)?'matched':''}`} disabled={locked&&!open.includes(card.key)} onClick={()=>choose(card)} aria-label={visible?card.treasure.name:'뒤집힌 문화재 카드'}><span className="memory-inner"><span className="memory-back"><i/><b>문화재</b></span><span className="memory-front"><img src={card.treasure.image} alt={visible?card.treasure.name:''}/></span></span></button>})}</div><div className="memory-actions"><Button onClick={hint} disabled={locked}>힌트 보기</Button><Button variant="outline" onClick={restart}>다시 하기</Button></div><div className="heritage-feedback" aria-live="polite">{message}</div>{complete&&<div className="activity-complete"><h2>문화재의 모습을 기억해 짝을 모두 찾았어요!</h2><Button onClick={onNext}>활동 선택으로</Button></div>}</section>;
}

function MemorySelect({onBack,start}:{onBack:()=>void;start:(count:number)=>void}){
  return <section className="expedition-activity"><div className="expedition-heading"><Button variant="outline" onClick={onBack}>← 이전</Button><div><h2>문화재 메모리게임</h2><p>카드 수를 골라요.</p></div></div><div className="memory-levels"><button onClick={()=>start(6)}><img src="/heritage-items/cheomseongdae.png" alt=""/><strong>쉬움</strong><b>6종 · 12장</b><small>3행 × 4열</small></button><button onClick={()=>start(8)}><img src="/heritage-items/seokgatap.png" alt=""/><strong>도전</strong><b>8종 · 16장</b><small>4행 × 4열</small></button></div></section>;
}
...
export default function HeritageExpedition({home,soundOn,toggleSound}:Props){
  ...
  return <main className="activity heritage-expedition"><HeritageTop home={home} soundOn={soundOn} toggleSound={toggleSound}/>{activity==='menu'&&<ExpeditionMenu open={setActivity} completed={completed}/>} {activity==='hidden'&&<HiddenHeritage soundOn={soundOn} onBack={back} onComplete={()=>complete('hidden')} onNext={()=>setActivity('memory-select')}/>} {activity==='memory-select'&&<MemorySelect onBack={back} start={count=>{setMemoryCount(count);setActivity('memory')}}/>} {activity==='memory'&&<MemoryGame count={memoryCount} soundOn={soundOn} onBack={()=>setActivity('memory-select')} onComplete={()=>complete('memory')} onNext={back}/>}</main>;
}
