...
function HiddenHeritage({soundOn,onBack,onComplete,onNext}:{soundOn:boolean;onBack:()=>void;onComplete:()=>void;onNext:()=>void}){
  ...
  const find=(treasure:HeritageTreasure)=>{const isNew=!found.includes(treasure.id);if(isNew){setFound(value=>[...value,treasure.id]);playDingDongDaeng(soundOn)}setFocus(treasure.id);setMessage(`${treasure.name}: ${treasure.desc}`);stopKoreanSpeech();stopRecordedVoice();if(soundOn){const audio=new Audio(`/audio/heritage/${treasure.id}.wav`);recordedVoice.current=audio;void audio.play().catch(()=>speakKorean(`${treasure.name}. ${treasure.desc}`,true))}};
