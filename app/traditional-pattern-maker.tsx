...
function PatternCanvas({pattern,actions,color,tool,size,canvasRef,onStroke}:{pattern:TraditionalPattern;actions:StrokeAction[];color:string;tool:ColoringTool;size:number;canvasRef:React.RefObject<HTMLCanvasElement|null>;onStroke:(action:StrokeAction)=>void}){
  ...
  const pointFromClient=(clientX:number,clientY:number,pressure:number)=>{const canvas=canvasRef.current;if(!canvas)return null;const rect=canvas.getBoundingClientRect();return{x:Math.max(0,Math.min(canvas.width,(clientX-rect.left)/rect.width*canvas.width)),y:Math.max(0,Math.min(canvas.height,(clientY-rect.top)/rect.height*canvas.height)),pressure:pressure>0?pressure:.5}};
