...
function PatternCanvas({pattern,actions,color,tool,size,canvasRef,onStroke}:{pattern:TraditionalPattern;actions:StrokeAction[];color:string;tool:ColoringTool;size:number;canvasRef:React.RefObject<HTMLCanvasElement|null>;onStroke:(action:StrokeAction)=>void}){
  ...
  const pointFromClient=(clientX:number,clientY:number,pressure:number)=>{const canvas=canvasRef.current;if(!canvas)return null;const rect=canvas.getBoundingClientRect();const scale=Math.min(rect.width/canvas.width,rect.height/canvas.height);if(!Number.isFinite(scale)||scale<=0)return null;const drawnWidth=canvas.width*scale,drawnHeight=canvas.height*scale;const offsetX=(rect.width-drawnWidth)/2,offsetY=(rect.height-drawnHeight)/2;return{x:Math.max(0,Math.min(canvas.width,(clientX-rect.left-offsetX)/scale)),y:Math.max(0,Math.min(canvas.height,(clientY-rect.top-offsetY)/scale)),pressure:pressure>0?pressure:.5}};
  const pointsFrom=(event:React.PointerEvent<HTMLCanvasElement>)=>{const nativeEvent=event.nativeEvent;const coalesced=typeof nativeEvent.getCoalescedEvents==='function'?nativeEvent.getCoalescedEvents():[];const samples=coalesced.length?coalesced:[nativeEvent];return samples.map(sample=>pointFromClient(sample.clientX,sample.clientY,sample.pressure)).filter((point):point is DrawPoint=>point!==null)};
  ...
  const start=(event:React.PointerEvent<HTMLCanvasElement>)=>{if(!ready)return;const point=pointsFrom(event).at(-1);if(!point)return;event.preventDefault();event.currentTarget.setPointerCapture(event.pointerId);currentRef.current={points:[point],color,tool,size};const context=canvasRef.current?.getContext('2d');if(context)drawStroke(context,currentRef.current,actions.length+1)};
  const appendPoints=(event:React.PointerEvent<HTMLCanvasElement>)=>{const stroke=currentRef.current;if(!stroke)return;const previous=stroke.points.at(-1)!;const added:DrawPoint[]=[];let last=previous;for(const point of pointsFrom(event)){if(Math.hypot(point.x-last.x,point.y-last.y)<1)continue;added.push(point);last=point}if(!added.length)return;stroke.points.push(...added);const context=canvasRef.current?.getContext('2d');if(context)drawStroke(context,{...stroke,points:[previous,...added]},actions.length+1)};
  const move=(event:React.PointerEvent<HTMLCanvasElement>)=>{if(!currentRef.current)return;event.preventDefault();appendPoints(event)};
  const finish=(event:React.PointerEvent<HTMLCanvasElement>)=>{const stroke=currentRef.current;if(!stroke)return;if(event.type==='pointerup')appendPoints(event);currentRef.current=null;onStroke({...stroke,points:[...stroke.points]})};
...
function PatternPicker({onChoose}:{onChoose:(id:TraditionalPatternId)=>void}){
  return <section className="tp-picker"><div className="tp-heading"><h2>마음에 드는 전통문양을 골라 아름다운 색으로 꾸며 보아요.</h2><p>문양 카드를 한 번 눌러 시작해요.</p></div><div className="tp-pattern-grid">{traditionalPatterns.map(pattern=><button key={pattern.id} type="button" onClick={()=>onChoose(pattern.id)}><img src={pattern.preview} alt={`${pattern.name} 선 그림`}/><strong>{pattern.name}</strong></button>)}</div></section>;
}
