...
export default function TraditionalPatternMaker({home,soundOn,toggleSound}:Props){
  ...
  const choose=(id:TraditionalPatternId)=>{const next=patternById[id];setSelectedId(id);setActions([]);setConfirm(null);setMessage(`${next.name}을 골랐어요. 색을 고르고 칠할 곳을 눌러 보세요.`)};
  ...
  const returnToPicker=()=>{setSelectedId(null);setActions([]);setConfirm(null);setMessage('마음에 드는 문양을 골라 보세요.')};
