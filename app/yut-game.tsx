...
function YutSticks({faces,rolling,onThrow,disabled,label}:{faces:readonly Face[];rolling:boolean;onThrow:()=>void;disabled:boolean;label?:string}){
  ...
  return <section className="png-throw-zone"><div className="throw-mat"><div className="throw-mat-inner"><img className="mat-png" src={`${asset}yut_throw_mat.png`} alt="전통 윷 던지기 놀이판"/><div className={`png-sticks ${rolling?'rolling':''}`} onPointerDown={event=>{if(disabled)return;start.current=event.clientY;event.currentTarget.setPointerCapture(event.pointerId)}} onPointerUp={event=>{if(start.current!==null&&start.current-event.clientY>35)throwNow();start.current=null}}>{faces.map((face,index)=>{const spot=layout[index];const style={'--stick-x':`${spot.x}%`,'--stick-y':`${spot.y}%`,'--stick-r':`${spot.r}deg`,'--stick-delay':`${index*.05}s`} as CSSProperties;return <span key={index} style={style}><img src={`${asset}yut_${face}.png`} alt={face==='round'?'둥근 면':face==='backdo'?'백도 표시 면':'평평한 면'}/></span>})}</div></div></div><Button className="png-throw-button" onPointerUp={event=>{event.preventDefault();throwNow()}} onKeyDown={keyActivate(throwNow)} disabled={disabled}>{rolling?'윷이 날아가요':label||'윷 던지기'}</Button><small>버튼을 누르거나 윷가락을 위로 밀어요</small></section>;
}
