...
const consonantEndingNames=new Set<Name>(['꽃신','버선','고름']);
...
function ItemArt({item,small=false}:{item:Item;small?:boolean}){return <div className={`hb-item-art ${small?'small':''} ${item.focus?'is-part':''}`} data-item={item.name}><img src={item.image} alt={`${item.name} 그림`}/>{item.focus==='goreum'&&<svg className="hb-goreum-outline" viewBox="0 0 271 257" aria-hidden="true"><path d="M105 94 C115 91 124 97 128 101 C138 94 150 94 156 98 L156 114 C146 118 136 117 124 111 L117 207 L86 207 L90 113 C83 108 88 98 96 98 Z"/></svg>}{item.focus==='somae'&&<span className="hb-part-outline focus-somae" aria-hidden="true"/>}</div>}
